...
func Ginzap(logger *zap.Logger, timeFormat string, utc bool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(c.Errors) > 0 {
			prom.HTTPServerTimer.Timing(domain, int64(latency/time.Millisecond), path[1:])
			prom.HTTPServerCounter.Incr(domain, path[1:], strconv.FormatInt(int64(c.Writer.Status()), 10))
